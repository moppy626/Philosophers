...
/*   Updated: 2025/10/16 16:22:05 by mmachida         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	eating(t_philo *philo)
{
	...
	if (philo->id == philo->d->num_of_philo
		|| philo->id % 2 == 0 || get_num_of_meals(philo) > 0)
...
int	sleeping(t_philo *philo)
{
	...
	wait_micro_s(get_lastmeal_time(philo) + to_micros(philo->d->time_to_eat),
		to_micros(philo->d->time_to_sleep), philo->d);
	return (0);
}
